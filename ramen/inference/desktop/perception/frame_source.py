...
#: 配信側 (運営 bridge = PC2) の時計との差を推定する窓 [s]。窓の中の
#: 「送信時刻 − 受信時刻」の最大 (= 届くまでの遅れが最も小さかった sample) を差とする。
SENDER_CLOCK_WINDOW_S = 5.0
...
class ZmqFrameSource:
    ...
    **1 回だけ decode** して ndarray を渡す。packed stereo も ndarray 上で
    ...
    Args:
        endpoint: 運営 bridge の ZMQ endpoint (例 ``tcp://<ORIN_HOST>:5555``)。
    ...
    def __init__(
        ...
        *,
        ...
        # key ごとの「最後の JPEG」と「内容が変わった時刻」(Codex #11)。
        self._key_payload: dict[str, bytes] = {}
        ...
        # The organizer endpoints are latest-state streams.  Without CONFLATE,
        # JPEG decode stalls can leave hundreds of old frames queued while each
        # newly decoded frame is incorrectly timestamped as freshly received.
    ...
    def _build_frame(self, images: dict) -> tuple[Optional[np.ndarray], str, int]:
        ...
        # 単眼 fallback は契約上の必須キー (ego_view) だけ。無いときに dict の先頭
        # (手首カメラのこともある) を head として使ってはいけない。
        if self._image_key not in images:
            return None, "", 0
        key = self._image_key
        self._warn_fallback_once(key, sorted(images))
        mono = self._decode(images[key])
        if mono is None:
            return None, "", 0
        if self._stereo_view == "packed":
            # Preserve the packed 2W contract.  Returning a 640px mono image
            # makes the downstream left/right selector silently crop it to
            # 320px.  Duplicating is an explicit degraded stereo fallback and
            # matches the submission-side behavior.
            return (
                self._cv2.hconcat([mono, mono]),
                f"hconcat({key}, {key}) (単眼複製 fallback)",
                len(images[key]),
            )
        return mono, f"{key} (単眼 fallback)", len(images[key])
    ...
    def _warn_fallback_once(self, key: str, available: list) -> None:
        ...
        orchestrator は packed (1280x480) を前提にするため、本classは単眼を
        左右へ複製して2W契約を保つ。これは視差を捏造せず、少なくとも640pxの
        保証された ``ego_view`` 全体を両入力へ渡す明示的な縮退動作である。
        ...
        print(
            ...
            f"(配信キー: {available})。packed入力は左右同一像へ縮退する",
            file=sys.stderr,
        )

    def _used_image_keys(self, images: dict) -> tuple[str, ...]:
        if self._stereo_view == "single":
            return (self._image_key,) if self._image_key in images else ()
        if self._stereo_view == "packed" and all(
            key in images for key in ("ego_view_left", "ego_view_right")
        ):
            return ("ego_view_left", "ego_view_right")
        if self._stereo_view in {"left", "right"}:
            eye = f"ego_view_{self._stereo_view}"
            if eye in images:
                return (eye,)
        return (self._image_key,) if self._image_key in images else ()

    def _content_changed_ns(self, images: dict, used_keys: tuple[str, ...]) -> int:
        """使う key のうち、内容の更新が**最も古い**ものの時刻。

        左右をまとめた signature だと、片眼が凍結しても反対側が変われば全体が
        新しく見える。bridge が凍結画像に新しい時刻を付け続ける場合、JPEG の
        一致が唯一の信号なので key ごとに持つ。
        """
        now_ns = time.monotonic_ns()
        for key in used_keys:
            payload = bytes(images[key])
            if self._key_payload.get(key) != payload:
                self._key_payload[key] = payload
                self._key_changed_monotonic_ns[key] = now_ns
        return min(self._key_changed_monotonic_ns[key] for key in used_keys)

    @staticmethod
    def _stamp_ns(msg: dict, image_keys: tuple[str, ...] = ()) -> int:
        """Use timestamps belonging to this camera, never another dict entry."""
        ts = msg.get("timestamps")
        if isinstance(ts, dict) and ts:
            try:
                values = [float(ts[key]) for key in image_keys if key in ts]
                if not values:
                    return time.time_ns()
                # Packed stereo is only as fresh as its older eye.
                v = min(values)
    ...
    @staticmethod
    def _sender_wall_time_s(msg: dict, image_keys: tuple[str, ...]) -> Optional[float]:
        """配信側が付けた送信時刻 [s] (配信側の壁時計)。無ければ None。

        `_stamp_ns` と違い、無いときに自分の時計で埋めない (時計の差の推定に使うため)。
        """
        ts = msg.get("timestamps")
    ...
    def _note_sender_clock(
        ...
        """「送信時刻 − 受信時刻」を 1 sample 記録する (窓の外の sample は捨てる)。"""
        sender_s = self._sender_wall_time_s(msg, used_keys)
        if sender_s is None:
            return
        sample = sender_s - received_wall_s
        horizon_ns = received_monotonic_ns - int(SENDER_CLOCK_WINDOW_S * 1e9)
        with self._lock:
            self._sender_clock_samples = [
        ...
        if first:
            print(
                f"[ZmqFrameSource] sender clock offset ~ {sample:+.3f}s "
                "(sender - local); boundary actions are stamped on the sender clock",
                file=sys.stderr,
            )

    def sender_clock_offset_s(self) -> Optional[float]:
        """配信側 (運営 bridge = PC2) の壁時計 − この host の壁時計 [s] の推定。

        直近 ``SENDER_CLOCK_WINDOW_S`` の「送信時刻 − 受信時刻」の最大 (届くまでの
        遅れが最も小さかった sample)。運営 adapter は ``issued_at`` を PC2 の時計と
        比べて古い chunk を捨てる (``--max-chunk-age-s 1.0``) ので、Thor から送る
    ...
    def _recv_loop(self) -> None:
        ...
        while True:
            ...
            try:
                ...
                return
            # 送信側との時計の差は受け取った直後の時刻で測る (decode の時間を含めない)。
            ...
            if not isinstance(images, dict) or not images:
                continue
            used_keys = self._used_image_keys(images)
            if not used_keys:
                continue
            self._note_sender_clock(
                msg, used_keys, received_wall_s, received_monotonic_ns
            )
            with self._lock:
                self._received_count += 1
            ...
            content_changed_ns = self._content_changed_ns(images, used_keys)
            frame = FrameData(
                rgb=rgb,
                t=self._stamp_ns(msg, used_keys),
                # A bridge can keep publishing/re-timestamping a frozen capture.
                # Exact JPEG equality is then the only end-to-end freshness
                # signal; static live cameras still contain sensor noise.
                received_monotonic_ns=content_changed_ns,
            )
            with self._lock:
                if self._closed:
                    return
