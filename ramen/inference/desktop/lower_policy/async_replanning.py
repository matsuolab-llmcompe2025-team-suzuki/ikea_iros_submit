...
# 予測の「古さ」を数える単位 [tick/s]。profile の max_prediction_age_s は
# 30 Hz の制御 (= 学習データの 30 fps) で決めた秒数なので、tick 数に直して比べる。
#
# **壁時計で比べてはいけない** (Issue #159 B3b-1)。大会経路は 1 tick = 教師 1 step
# だけ進み、tick は最大 20 Hz で推論に律速される。採用は tick で決まる
# (lead_steps 後) のに古さだけ壁時計で測ると、推論が一瞬でも「lead ÷ tick レート」
# 秒は経つ。GR00T (lead 4 / 0.35 s) は約 14 Hz、RAMEN-Ori (lead 3 / 0.25 s) は
# 約 12 Hz を下回ると**毎回の予測が古いとして捨てられ**、最後の target を黙って
# 保持し続けていた。
#
# tick で数えれば「その予測は何 step 前の観測から作られたか」になり、両経路で
# 同じ意味になる (30 Hz の自前経路では今までと同じ判定)。推論が本当に遅れた
# ときは tick が進み続けるので、今までどおり捨てられる。
PREDICTION_AGE_TICKS_PER_S = 30.0
...
class AsyncActionChunkPipeline:
    ...
    def __init__(
        ...
        *,
        ...
        self._max_prediction_age_ticks = max_prediction_age_s * PREDICTION_AGE_TICKS_PER_S
        self._index = 0
        # next_action() の呼び出し回数 = 経過 tick。予測の古さはこれで測る。
        self._ticks = 0
        ...
        self.last_stale_discard_age_ticks: int | None = None
    ...
    def submit(
        ...
        *,
        ...
        self._pending_submitted_tick = self._ticks
    ...
    def promote_if_ready(self) -> CompletedPrediction | None:
        ...
        submitted_tick = self._pending_submitted_tick
        self._pending = None
        self._pending_submitted_monotonic_ns = None
        self._pending_submitted_tick = None
        self._thread = None
        age_ticks = self._ticks - (submitted_tick if submitted_tick is not None else self._ticks)
        if age_ticks > self._max_prediction_age_ticks:
            self.stale_discard_count += 1
            self.last_stale_discard_age_ticks = int(age_ticks)
            self.last_stale_discard_age_ms = (
                time.monotonic_ns() - completed.submitted_monotonic_ns
            ) / 1.0e6
            return None
