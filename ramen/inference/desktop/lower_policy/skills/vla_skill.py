...
class VlaSkill(Skill):
    ...
    def step(self, obs: dict) -> np.ndarray | None:
        ...
        if self._dispatch_waist and bool(getattr(self._policy, "outputs_waist", True)):
    ...
    def _flush_execution_state(self, *, reset_motion_limiter: bool = True) -> None:
        ...
        **progress monitor は reset しない**。参照を attempt ごとに張り直すと、
        ...
        Args:
            reset_motion_limiter: False なら MotionLimiter は前の指令を基準に保つ。
                やり直しの境目では前の指令が古いので実測から始め直す (True)。指令が
                途切れずに続く境目 (hybrid の区間 2→3) では False にする。
        """
        reset_fn = getattr(self._policy, "reset", None)
        if callable(reset_fn):
            reset_fn()
        if reset_motion_limiter and self._motion_limiter is not None:
            self._motion_limiter.reset()
        self._action_queue.clear()
        self._action_queue_next_index = 0
        self._attempt_start_z_m = None
    ...
    def _build_raw_state(
        ...
        if joint_state is not None:
            ...
            if position is not None:
                ...
                if arr.shape == (29,):
                    joint_positions = arr
            # `base_quat_wxyz` (公式 :5557 の IMU 姿勢) は model 入力に使わない。
            # yaw は起動ごとに任意で dataset の world frame と対応しておらず、
            # 実機検証済みの入力は SDK 経路と同じ直立 proxy (policy 側の既定) のため。
            # 経路によって model 入力を変えない。
