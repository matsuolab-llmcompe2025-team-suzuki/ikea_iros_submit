...
def _step_of(filename: str) -> int:
    ...
    return int(Path(filename).name[len("ckpt_step_") : -len(".pt")])


def resolve_hf_ckpt(
    repo_id: str, revision: str | None, ckpt_filename: str | None
) -> str:
    """HF repo の RAMEN-Ori ckpt を**ネット無しでも**解決する (Issue #159 B3d-03)。

    会場の実行時はネットに繋がない (`HF_HUB_OFFLINE=1`)。以前は読み込みのたびに
    `HfApi.list_repo_files` を無条件に呼んでいて、GR00T や YOLO は cache から読めるのに
    RAMEN-Ori (rotate_table_base の既定、1 run で 4 回読み直す) だけ落ちていた。

    - `ckpt_filename` が決まっていれば (slot の `ckpt_filename`) HF API を呼ばない。
      `hf_hub_download` は commit を固定していれば cache から返す
    - 決まっていなければ、まず cache の中の最新の `ckpt_step_*.pt` を使い、cache に
      無いときだけ repo の一覧を HF API に聞く
    """
    from pathlib import Path
    ...
    try:
        cached_root = Path(
            snapshot_download(
                repo_id,
                revision=revision,
                allow_patterns=["ckpt_step_*.pt"],
                local_files_only=True,
            )
        )
        cached = sorted(
            cached_root.glob("ckpt_step_*.pt"), key=lambda path: _step_of(path.name)
        )
    except Exception:  # noqa: BLE001 - cache に無いだけ。下で API に聞く
        cached = []
    if cached:
        return str(cached[-1])
    try:
        files = HfApi().list_repo_files(repo_id=repo_id, revision=revision)
    except Exception as exc:  # noqa: BLE001 - オフライン等
        raise FileNotFoundError(
            f"no cached ckpt_step_*.pt for {repo_id}@{revision} and the HF API is "
            f"unreachable ({type(exc).__name__}). Pin ckpt_filename in the slot and "
            "prefetch that file before the venue (see venue_commands.md)"
        ) from exc
    step_ckpts = sorted(
    ...
    if not step_ckpts:
        raise FileNotFoundError(f"no ckpt_step_*.pt in HF repo {repo_id}@{revision}")
    return hf_hub_download(repo_id=repo_id, filename=step_ckpts[-1], revision=revision)
...
class RamenOriPolicy:
    ...
    def __init__(
        ...
        # 腰を出す ckpt か (19D 学習) / 出さない ckpt か (16D 学習、推論側が 0 を pad)。
        # `outputs_waist` が読む。model は読み込み済みなので初回 predict を待たずに決まる。
        self._trained_action_dim: int | None = (
            int(_model.action_dim) if hasattr(_model, "action_dim") else None
        )
    ...
    def outputs_waist(self) -> bool:
        """返す 19D chunk の腰 (行 [0:3]) が model 自身の出力か (Issue #144)。

        16D で学習した ckpt (Phase K / Issue #141 の再学習) は腰を出さないので、
        推論側が 0 を pad して 19D にしている。その 0 をそのまま actuator に流すと
        腰が直立 (0 rad) へ寄っていくので、VlaSkill 側で捨てる必要がある。
        19D で学習した ckpt (Phase 1/2 の 5 variant) は腰も出すので流してよい。

        `dispatch_waist` は skill 単位の設定 (skill_config.yaml) で、そこには
        「どの ckpt を載せたか」が出てこない。腰を出すかは ckpt の性質なので、
        skill の設定ではなく policy が答える。

        判定できないとき (model が action_dim を持たない stub など) は、腰を
        止める側に倒す: hold は rotate_table_base で常用している安全な状態で、
        誤って 0 を流すより副作用が小さい。
        """
        dim = self._trained_action_dim
        if dim is None:
            dim = self._model_action_dim
        return dim == 19
    ...
    def predict(self, obs: Observation) -> PolicyAction:
        ...
        if self._pipeline is None:
            ...
            if promoted is not None and self._pending_submit_step is not None:
                ...
                # 古いとして捨てられた予測は pipeline が消すので、こちらの latch も
                # 外して次の要求をすぐ出せるようにする (GR00T と同じ、Issue #159
                # B3d-01)。外さないと wants_prediction が立っても submit せず、
                # その skill の間 async 再計画が止まり、seed を使い切るたびに
                # tick を塞ぐ同期推論 (sync_fallback) だけで動いていた。
