...
def _gpu_free_bytes_for_load(torch_module: object) -> int:
    """GR00T を読む前の GPU の空き。統合 GPU (Thor など) では OS の MemAvailable を使う。

    CPU と GPU が同じメモリの統合 GPU では、cudaMemGetInfo の free はすぐ空けられる page cache を
    使用中に数えて少なく出る (NVIDIA CUDA for Tegra app note「Estimating total allocatable device
    memory on an integrated GPU device」。vLLM 0.29 の MemorySnapshot も同じ理由で psutil の
    available に替える)。会場は run ごとに VLM 17 GB などの重みを読むので page cache が溜まり、
    実際は空いているのに 53D の読み込みを止めてしまう (B3b-4)。独立 GPU (開発機) は従来どおり。
    """
    free, total = torch_module.cuda.mem_get_info()
    device = torch_module.cuda.current_device()
    if not getattr(torch_module.cuda.get_device_properties(device), "is_integrated", False):
        return int(free)
    available = _host_mem_available_bytes()
    print(
        "[groot] integrated GPU: load headroom from MemAvailable="
        + ("unknown" if available is None else f"{available / 1024**3:.1f}GiB")
        + f" (cudaMemGetInfo free={free / 1024**3:.1f}/{total / 1024**3:.1f}GiB)",
        file=sys.stderr,
    )
    return int(free) if available is None else int(available)
...
class _GrootWorkerClient:
    ...
    def __init__(self, cfg: PolicyConfig) -> None:
        ...
        try:
            self._connection = self._wait_for_worker_socket()
        except BaseException:
            # 起動待ちの Ctrl+C でも worker を落とす (別 session なので SIGINT は届かない)
            self.close()
            raise

    def _wait_for_worker_socket(
        self, *, poll_s: float = 0.1, progress_s: float = 10.0
    ) -> socket.socket:
        """worker が model を読み終えて socket を開くまで待ち、接続を返す。

        worker は model を読んでから socket を作るので、この待ちは model の読み込みを含む。
        時間制限は付けない (読み込みは待つ。以前の 300 s は Thor の初回の読み込みで
        切れうる、B3c-2)。worker が終わったら故障として止める。経過は progress_s ごとに出す。
        """
        ...
        while True:
            ...
            if self._socket_path.is_socket():
                ...
                # 1 回の要求 (推論) の応答待ちの上限。worker の次の要求待ちとは別。
            ...
            if now >= next_progress:
                print(
                    f"[groot] waiting for the GR00T worker to load... "
                    f"{now - started:.0f}s (no time limit; Ctrl-C stops the run)",
                    file=sys.stderr,
                )
...
class Gr00tPolicy:
    ...
    def from_ckpt(cls, cfg: PolicyConfig) -> "Gr00tPolicy":
        ...
        if str(cfg.device).startswith("cuda"):
            ...
            gpu_free_bytes = _gpu_free_bytes_for_load(torch)
