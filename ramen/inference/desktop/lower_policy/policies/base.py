...
class RawRobotState:
    ...
    **設計方針** (Issue #125 Phase A-1): inference 側は Orin (real_hw_bridge_node)
    ...
    38D full-body)。lab DDS経路にはroot poseが無いが、公式boundaryはbase quaternion
    を配るため、利用できる場合だけstationary-root proxyの姿勢へ反映する。
...
class PolicyConfig:
    ...
    def __post_init__(self) -> None:
        ...
        if self.ckpt_filename is not None:
            name = PurePosixPath(self.ckpt_filename)
            if (
                not self.ckpt_filename
                or name.is_absolute()
                or any(part in {"", ".", ".."} for part in name.parts)
            ):
                raise ValueError(
                    "ckpt_filename must be a non-empty relative POSIX path "
                    f"without traversal, got {self.ckpt_filename!r}"
                )
...
class Policy(Protocol):
    ...
    任意の属性 `outputs_waist` (bool): 返す 19D chunk の腰 [0:3] が model 自身の
    出力か。持たない実装は True 扱い (GR00T / ACT / DP は腰も学習して出す)。
    False を返すのは推論側が 0 を pad した場合で、VlaSkill はその腰を送らない
    (Issue #144、RAMEN-Ori の 16D 学習の ckpt)。
