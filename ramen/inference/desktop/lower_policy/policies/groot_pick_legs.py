...
def build_state_from_raw(raw: RawRobotState) -> np.ndarray:
    ...
    # Position is unavailable, so retain the proven stationary 0.70m proxy.
    # The official boundary does provide base orientation; use it when present
    # instead of silently forcing an upright quaternion.
    root_pose_pad = (
        np.asarray(raw.root_pose, dtype=np.float32)
        if raw.root_pose is not None
        else np.asarray((0.0, 0.0, 0.70, 1.0, 0.0, 0.0, 0.0), dtype=np.float32)
    )
    if root_pose_pad.shape != (7,) or not np.isfinite(root_pose_pad).all():
        raise ValueError("root_pose must be finite 7-D when provided")
