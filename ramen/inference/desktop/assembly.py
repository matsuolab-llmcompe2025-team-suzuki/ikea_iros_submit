...
# ここまで届くのは補正でも止まらなかったときだけ = 最後の安全網。教師の範囲はこの
# 限界と交わらせてから使う (`teacher_range_within_limits`、Issue #159 B4b-01)。
...
def build_yolo_perception(
    ...
    重みは `repo@commit` と repo の中の path で指定し、HF の cache から読む。学習の overlay を焼いた
    ...
    weight_path = resolve_yolo_ckpt_ref(settings.ckpt_ref, settings.ckpt_file)
    print(
        f"[assembly] YOLO: {settings.ckpt_ref} file={settings.ckpt_file} conf={settings.conf} "
...
def build_sensor_sources(
    *,
    ...
    DDS-backed sources require ``ChannelFactoryInitialize`` to have run first.
    The official ``boundary``/``zmq`` combination is pure ZeroMQ and must not
    initialize DDS at all.
    ...
    Args:
        ...
        joint_source: 関節角の取り方 (INF-18)。
            ...
            - "boundary": 運営公式の ZeroMQ ``:5557`` (会場 boundary 専用)
    ...
    if joint_source == "boundary" and synthetic_hand_initial_rad is None:
        # 公式 boundary に Dex1 state は無い。rt/dex1 (DDS) を読みに行かない。
        # source を 1 つも作る前に落とす (receiver thread を残さない)。
        raise ValueError(
            "joint_source='boundary' requires synthetic_hand_initial_rad; "
            "the official boundary publishes no Dex1 state"
        )
    if joint_source == "lowstate":
        ...
        raise ValueError(
            "joint_source must be 'lowstate', 'joint_states', or 'boundary', "
            f"got {joint_source!r}"
        )
    if head_source == "zmq":
        from inference.desktop.perception.frame_source import ZmqFrameSource

        if not zmq_endpoint:
            raise ValueError("head_source='zmq' requires zmq_endpoint")
    ...
    if synthetic_hand_initial_rad is None:
        ...
        if joint_source == "boundary":
            # 運営 state bridge が `gripper_q` を載せていれば実測、無ければ合成
            # (Codex #2、2026-09-23 決定)。
            from inference.desktop.perception.boundary_state_source import (
                BoundaryDex1StateSource,
            )

            dex1 = BoundaryDex1StateSource(joint, dex1)
...
def teacher_range_within_limits(teacher_range: Any, skill_name: str) -> Any:
    """教師の範囲を MotionLimiter の腕の範囲 (URDF − `POSITION_MARGIN_RAD`) と交わらせる。

    教師が URDF の限界まで使った関節 (手首 ±1.6144 等) は、教師の端 = 限界そのもの。
    そのままだと (限界 − 0.03, 教師の端] の帯で補正が効かずに MotionLimiter が 19D を
    丸ごと保持し、範囲を出ても引き戻し先がまた拒否される (Issue #159 B4b-01。rotate_leg は
    教師の 10% の episode がこの帯に入る)。交わらせれば、その関節だけ限界 − 0.05 に
    抑えられ、上半身は動き続ける。削った関節は起動時に出す。
    """
    ...
    narrowed_range, narrowed = teacher_range.within_limits(
        G1_ARM_POSITION_LOWER_RAD + POSITION_MARGIN_RAD,
        G1_ARM_POSITION_UPPER_RAD - POSITION_MARGIN_RAD,
    )
    if narrowed:
        print(
            f"[assembly] {skill_name}: teacher range clipped to the limiter "
            f"(URDF - {POSITION_MARGIN_RAD:g}) on {list(narrowed)}",
            file=sys.stderr,
        )
...
def _build_motion_limiter(
    ...
    安全網で、通常は教師の範囲の補正 (`teacher_range.py`) が先に効く (教師の範囲は
    `teacher_range_within_limits` でこの限界と交わらせてある)。
    """
    from inference.desktop.lower_policy.actuators.g1_arm_sdk import (
        G1_ARM_POSITION_LOWER_RAD,
        G1_ARM_POSITION_UPPER_RAD,
...
def build_vla_skill(
    *,
    ...
    if teacher_range is not None:
        teacher_range = teacher_range_within_limits(teacher_range, skill_name)
...
def head_procedure_skill_names(
    ...
    """
    names: list[str] = [f"wait_for_go_live_{skill_name}"]
    if include_hand:
...
def hold_sec_from_config(skill_config: dict) -> float:
    """頭の手順・境界の最後に開始姿勢で保持する秒数 (`hold_pose.hold_sec`)。

    正本は YAML (Issue #159 T9、B4a-14)。自前経路の `--setup-hold-sec` は指定した
    ときだけこれを上書きし、大会経路の driver はここを読む。
    """
    section = dict(skill_config.get("hold_pose") or {})
    unknown = sorted(set(section) - {"hold_sec"})
    if unknown:
        raise ValueError(f"hold_pose has unknown keys: {unknown}")
    hold_sec = float(section.get("hold_sec", 3.0))
    if not np.isfinite(hold_sec) or hold_sec < 0.0:
        raise ValueError(f"hold_pose.hold_sec must be finite and >= 0, got {hold_sec}")
    return hold_sec
...
def build_head_procedure(
    *,
    ...
    ロボットが指令に従い始めるのを待つ (go-live、Issue #159 T5) → 手を開く →
    腕の pre-motion → 手を開始の開度へ → N 秒保持。
    ...
    go_live = dict(skill_config.get("go_live_wait") or {})
    unknown = sorted(set(go_live) - {"lift_rad", "detect_rad", "flag_paths"})
    if unknown:
        raise ValueError(f"go_live_wait has unknown keys: {unknown}")
    skills: list = [
        GoLiveWaitSkill(
            name=names[0],
            lift_rad=float(go_live.get("lift_rad", 0.05)),
            detect_rad=float(go_live.get("detect_rad", 0.02)),
            flag_paths=tuple(go_live.get("flag_paths") or ()),
        )
    ]
    if include_hand:
        skills.append(
    ...
    unknown = sorted(
        ...
        - {
            ...
            "stage_timeout_s",
            "stage_wall_timeout_s",
            "boundary_lift_rad",
        }
    )
    if unknown:
        raise ValueError(f"arm_pre_motion has unknown keys: {unknown}")
    # 頭の手順は歩行の後に初めて腕を上げるので、頭上・側方の退避のまま。
    # `boundary_lift_rad` は model の境界だけで使う (Issue #152)。
    arm_settings.pop("boundary_lift_rad", None)
    skills.append(
        CollisionAwareArmPreMotionSkill(
            tuple(initial_pose.arm_position_rad.tolist()),
            skill_name=f"arm_pre_motion_for_{skill_name}",
...
def build_model_transition_procedure(
    *,
    ...
    Unlike a stage-start head procedure this does **not** open the hands fully.
    On the carrying boundary (`KEEP_GRIP_MODEL_TRANSITIONS`, pick -> insert) the
    previous Dex1 target remains published while the arms move.  On every other
    boundary only a hand that is still gripping is released before the arms move
    (Issue #159 T3).  The next model's recorded frame-zero hand target is applied
    only after the arms have measurably converged.
    """
    ...
    if unknown:
        raise ValueError(f"arm_pre_motion has unknown keys: {unknown}")
    skills: list = []
    if names[0].startswith("hand_release_"):
        # 腕を上げる前に、握ったまま切られた手を離す (Issue #159 T3)
        skills.append(
            HandPreMotionSkill.from_config(
                skill_config,
                next_skill,
    ...
    # 腕を上げる頭上・側方の退避は、skill 間の移動には広すぎる (脚を保持した腕を
    # roll ±1.5 まで開くと、持った脚がテーブルや反対の腕を薙ぐ)。
    arm_name = f"arm_transition_{previous_skill}_to_{next_skill}"
    skills.append(
        CollisionAwareArmPreMotionSkill(
            tuple(initial_pose.arm_position_rad.tolist()),
            skill_name=arm_name,
    ...
    print(
        f"[assembly] model boundary {previous_skill} -> {next_skill}: "
        f"steps={names} waypoints={list(skills[-1].waypoint_names)}",
        file=sys.stderr,
    )
    if include_hand:
        grasp = bool(initial_pose.requires_separate_hand_initialization)
