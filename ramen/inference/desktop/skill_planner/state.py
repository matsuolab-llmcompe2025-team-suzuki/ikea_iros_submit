...
class SkillState:
    ...
    Attributes:
        ...
        n_legs_completed: 完了 leg 数 (0..4)。transition() 内で prev ==
            ...
            enter_pick_table_leg (実機 monitor、旧 replay のみ n=0 → Kabsch /
            n>=1 → aspect) /
        ...
        base_rotation_start_table_top_verts: 直近 move_table_base 遷移開始時の
            ...
            enter_pick_table_leg の旧 replay Kabsch / aspect fallback の reference。
    ...
    # rotate_table_base の learned skill が出す、教師データで校正済みの
    # RotateProgressMonitor の結果。Planner 側の旧 aspect-ratio proxy と区別して
    # 保持し、monitor が存在する実機経路ではこちらを authoritative にする。
    table_rotation_monitor_available: bool = False
    table_rotation_usable: bool = False
    table_rotation_sustained: bool = False
    table_rotation_deg: float = 0.0

    def update_table_rotation_progress(self, metadata: dict) -> None:
        """直近 policy action の天板回転診断を Planner state へ反映する。

        metadata に progress key が無い旧 policy では何も変更しない。その場合だけ
        `enter_pick_table_leg` は旧 aspect-ratio 判定へ fallback できる。
        """
        required = {
            "table_rotation_deg",
            "table_rotation_sustained",
            "table_rotation_usable",
        }
        if not required.issubset(metadata):
            return
        rotation_deg = float(metadata["table_rotation_deg"])
        if not np.isfinite(rotation_deg):
            return
        self.table_rotation_monitor_available = True
        self.table_rotation_deg = rotation_deg
        self.table_rotation_usable = bool(metadata["table_rotation_usable"])
        self.table_rotation_sustained = bool(metadata["table_rotation_sustained"])
    ...
    def transition(self, new_skill: str, ctx: Optional[dict] = None) -> None:
        ...
        if new_skill == "rotate_table_base":
            ...
            self.table_rotation_monitor_available = False
            self.table_rotation_usable = False
            self.table_rotation_sustained = False
            self.table_rotation_deg = 0.0
