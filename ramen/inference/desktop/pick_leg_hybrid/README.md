...
| 3 | 左右で持ち替え、insertのframe-0姿勢へ接続 | rule-based FSM（既定） | 把持・解放・初期姿勢到達 |
...
運営側は受け取った手先目標をIKで実行する。本体のboundary経路でも共通skillはarm14を
経由し、送信時のFKでzero-offset手首姿勢へ変換する。`RealPickLegHybridVlaSkill` はCartesian補間を
...
pickはこのhybridが既定。従来のVLA単体を比較するときだけ `--no-pick-leg-hybrid` を明示する。
VLM不通や参照画像不足では指令開始前に停止し、VLA単体へ自動で切り替えない。他skillは変更しない。

Phase 3はルールベースが既定。単体評価でGR00Tへ戻す比較版だけ、次を追加する。

```bash
--pick-leg-phase3-executor vla
```

既定のrule_based版はPhase 2終点から、左把持、右解放、左での提示、右再把持、左解放、
...
単体`run_skill`でも本番stage実行でも、executor省略時は`rule_based`。
本番stage実行は有限完了契約を必須にし、`vla` Phase 3を拒否する。

## 本番 Stage 1〜4 への差し替え

本番のstage実行は`inference.desktop.entrypoint`が入口で、SDK/boundaryとも
Stage 1〜4の`pick_table_leg`は既定で上記の有限`rule_based` Phase 3版になる。
pick完了時には`skill_config.yaml:skills.insert_table_leg.initial_pose`の腕14Dと
Dex1 2Dへ実測到達したら`insert_table_leg`へ遷移する。hybrid全体が30秒を超える、
または持ち替えの段が締め切りまでに届かない場合は、時間切れとしてpickを終えて次skillへ
進む（次へ進む道はYOLOと時間切れだけ、止めるのは人）。IKが解けない間と支えの把持が
外れた疑いのときは記録だけ残して続ける。故障（Dex1が動かない、実測が来ない等）の
ときだけ最後の安全targetを保持してpolicyを停止し、本番entrypointは共通終了処理で
手を開き、腕を下ろしてcontrolled releaseする（終了処理の失敗時も解放する）。
この終了仕様は単体評価runnerの安全停止時Enter待ちとは異なる。
...
(CLIが勝つ)。旧learned-only pickを比較するときは`--no-pick-leg-hybrid`を明示する。
会場側の起動と提出物への反映状況は[venue_commands.md](../../../docs/handoff/venue_commands.md)を参照する。
...
ローカルserverは`run_local_vlm_server.sh`で起動する。提出image（Thor）とRunPodでは同じcontainerの中で
`run_venue_vlm_server.sh`（vlm環境のvLLM、同じmodel名・port・上限）を使う。entrypointに`--spawn-vlm-server`を
付けると、hybridを使うrunだけこのscriptを子processとして起動し、読み込みを待って慣らしを1回送り、runの終わりに
止める（`vlm_server.py`）。production requestは参照2枚、
