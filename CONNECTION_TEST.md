...
- [ ] **その image の中身（本体の commit）を確かめる。** submit repo の `ramen/RAMEN_SOURCE.txt`。本体 `9849a17` より前の image には `--wrist-roll-clamp` と `--walk-lowering-check` が無いので、下の R1b と R4 は試せない。
...
| 2-2 | **PC2 の運営 package の版** | PC2 で `git -C <package> log -1 --format='%h %ci'`（git で入れていなければ運営に聞く）。**`609f61d`（2026-09-25）以降か** | 古いと、頭の画像が潰れる（2-3）、WBC の起動時に腕が動く（§3 の Step 2）、joint lane が無い（R3 は試せない） |
...
| 4 Thor | 読み込みの秒（Stage 1〜4 は Enter 1 まで 4〜5 分の目安）、`vlm_latency`（5 秒以内でないと止まる）、`[groot] integrated GPU: … MemAvailable=…`、`sender clock offset ~ …`、`[init] policy variants: …`、`[boundary] … (pose lane)` か `(joint lane)` |
...
| **R2** | 1（脚 1 本目） | なし（既定 = pose lane） | 今の本番の形で 1 本通す | 台を回す → pick（VLM が答えるか、握れるか、持ち替え）→ insert → 締め付け。Enter 2 の問い（下の注）。`[orch] … timed out short of its target` の行の数。adapter の IK の失敗（reject）の多さ |
| **R3** | 1 | `--boundary-lane joint` | 関節角をそのまま送る形（2-2 が 609f61d 以降のときだけ） | R2 と同じ所を比べる。開始姿勢への到達（`joint_error`）、動きが学習どおりに見えるか、adapter の `[stats]` に joint の受信が数えられるか、clamp の log が多すぎないか |
| R4 | 1 | `--wrist-roll-clamp off` | pose lane を使う場合の手首 roll（2-2 が 609f61d 以降のときだけ） | 締め付け（手首 roll を大きく使う）で腕が止まらないか。起動 log の `[boundary] wrist_roll clamp: off …` |
| **R5** | 5 | R2・R3 で良かった方の送り方 | flip | 開始姿勢 → Enter 2 → 裏返す |
...
| 決めること | 選択肢 | 決め方 |
|---|---|---|
| 指令の送り方（`--boundary-lane`） | `pose`（今の既定）/ `joint` | 2-2 が 609f61d 以降で、R3 が R2 より開始姿勢によく届き、腕が止まらず、動きが学習どおりなら `joint`。どれかが怪しければ `pose` |
| 手首 roll の clamp（`--wrist-roll-clamp`、pose lane のときだけ） | `on`（今の既定）/ `off` | pose lane を選び、2-2 が 609f61d 以降で、R4 で腕が止まらなければ `off`。それ以外は `on` |
...
| R1 | | 0 | — | | | | |
| R2 | | 1 | — | | | | |
| R3 | | 1 | `--boundary-lane joint` | | | | |
| R4 | | 1 | `--wrist-roll-clamp off` | | | | |
| R5 | | 5 | | | | | |
...
| | pose（既定） | joint |
|---|---|---|
| 送るもの | 手先の位置と向き (T,25)。運営 IK が関節角に戻す | 腕の関節角 (T,22)。運営 IK を通らない。手・歩行・骨盤高さの列は pose と同じ値 |
...
- pose lane で、手首 roll を運営 IK の古い上限（±0.9、余裕を見て ±0.88）に寄せるか。既定 `on`。
- 運営 IK の `a1af470`（package `609f61d`）でこの上限は無くなった。学習データは 0.9 を超える手首 roll を使うので、新しい IK なら `off` の方が学習どおりに動く。古い IK に `off` で送ると、0.9 を超えた目標でその腕が丸ごと止まる。
- 起動 log の `[boundary] wrist_roll clamp: on …` / `off …` で分かる。joint lane と sdk 経路では `off` を指定すると起動時に止める（効かないため）。
