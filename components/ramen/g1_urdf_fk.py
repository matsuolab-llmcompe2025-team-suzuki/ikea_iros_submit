...
VENDORED from iros_2026_ramen `inference/desktop/perception/g1_urdf_fk.py`
(self-contained Thor image のため submission repo に複製)。pure numpy。

⚠️ **`tools/sync_vendor_desktop.sh` の対象外＝手動コピー。** 上流を更新したら
ここも同期すること。同期漏れは `tests/test_fk_copies_agree.py` が検出する
(2026-09-22 に tool offset が 10cm ずれたまま放置されていたのを検出、Issue #7)。
...
# VENDOR 適応: URDF は module 相対 `assets/` に置く (container の CWD 非依存)。
# 上流は repo root からの相対 (parents[3]) だが、container では /app が CWD で
# 解決できず FileNotFoundError になり **全 53D skill が起動失敗する** (IAC eval 指摘)。
# ここが、この手動コピーが存在する唯一の理由。
DEFAULT_URDF_PATH: str = str(
    Path(__file__).resolve().parent / "assets" / "g1_29dof_mode_15_with_dex1_1.urdf"
)
